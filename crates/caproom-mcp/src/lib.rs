...
static GROWTH: std::sync::OnceLock<std::sync::Mutex<caproom_core::growth::GrowthRing>> =
...
pub fn handle_top(pid: Option<i32>) -> TopResponse {
    ...
    let roots = if let Some(p) = pid {
        vec![p]
    } else {
        caproom_core::process_tree::Tree::roots(&snap)
    };
    ...
    for r in roots {
        if let Some(t) = caproom_core::process_tree::Tree::build(r, &snap) {
            ...
            let park_candidate =
                state == State::Running && is_sleeping && t.footprint_kb >= 512 * 1024;
            let growth_kb_s = ring.update(r, t.footprint_kb);
            let reason_code =
                match caproom_core::growth::reason_code(park_candidate, growth_kb_s, free_pct) {
            ...
            for p in &tree.pids {
                ...
                if let Some(orig) = snap.by_pid(*p) {
                    if orig.start_time != 0 {
                        if let Some(cur) =
                            caproom_core::collector::snapshot_current_user().by_pid(*p)
                        {
...
pub fn handle_wake(pid: i32) -> serde_json::Value {
    ...
    #[cfg(unix)]
    {
        ...
        if r == 0 {
            serde_json::json!({"woken": [pid], "state": "running"})
        } else {
            serde_json::json!({"error": format!("no such pid {}", pid)})
        }
    }
...
pub fn handle_run(command: Vec<String>, limit_mb: u64) -> serde_json::Value {
    ...
    let caproom_bin = std::env::current_exe()
        .ok()
        .and_then(|p| p.parent().map(|d| d.join("caproom")))
        .unwrap_or_else(|| std::path::PathBuf::from("caproom"));
    let mut cmd = std::process::Command::new(&caproom_bin);
    cmd.args(["run", "--limit", &limit_mb.to_string(), "--"])
        .args(&command);
    let out = cmd.output();
    match out {
        Ok(o) => {
...
mod tests {
    ...
    fn schema_enums_are_typed_no_freeform_message() {
        ...
        assert!(
            !s.contains("\"message\""),
            "freeform message field must not exist"
        );
        // state must be enum, not arbitrary string
        for p in &r.processes {
            let _ = serde_json::to_value(&p.state).unwrap();
        }
    }
    #[test]
    ...
    #[test]
    fn handle_top_missing_pid_returns_empty_list() {
        let r = handle_top(Some(-1));
