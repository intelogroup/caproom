...
fn risk_score(
    ...
    let pressure_term = if free_pct < 20 {
        (20 - free_pct) as f64 * 10.0
    } else {
        0.0
    };
    let fanout_term = fanout as f64 * 2.0;
    let urgency_term = secs_to_crit
        .map(|s| if s < 300.0 { (300.0 - s) * 0.5 } else { 0.0 })
        .unwrap_or(0.0);
    (
        footprint_term + growth_term + pressure_term + fanout_term + urgency_term,
        secs_to_crit,
    )
}

fn main() {
    let limit_mb: u64 = std::env::var("CAPROOM_LIMIT_MB")
        .ok()
    ...
    eprintln!(
        "limit={}MB interval={:?} — Ctrl-C to stop",
        limit_mb, interval
    );
    ...
    loop {
        ...
        if iter.is_multiple_of(5) || sessions.iter().any(|s| s.footprint_kb >= eff_kb) {
            ...
            for s in sessions.iter().take(5) {
                eprintln!(
                    " pid {:>6} risk {:>6.1} {}MB g {:>5}KB/s fanout {} crit {:?}s -- {}",
                    s.pid,
                    s.risk,
                    s.footprint_kb / 1024,
                    s.growth_kb_s,
            ...
            if let Some(worst) = sessions.first() {
                if worst.footprint_kb >= eff_kb {
                    ...
                    eprintln!(
                        "caproomd: high risk {:.1} — would warn agent pid {} before park",
                        worst.risk, worst.pid
                    );
                }
            }
        }
