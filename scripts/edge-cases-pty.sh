...
# portable script pty helper
run_script_pty() {
  # $1 = log prefix, $2... = command
  local prefix="$1"; shift
  if [[ "$(uname)" == "Darwin" ]]; then
    script -q /dev/null "$@" > "${prefix}.log" 2> "${prefix}.err"
  else
    # linux: script -q -c "cmd" /dev/null
    local cmd
    cmd=$(printf '%q ' "$@")
    script -q -c "$cmd" /dev/null > "${prefix}.log" 2> "${prefix}.err"
  fi
  return $?
}
...
# tty opencode should bypass (detached watch)
if run_script_pty /tmp/edge1 bin/caproom --limit 512 -- opencode; then
...
if run_script_pty /tmp/edge2 bin/caproom --limit 512 -- myapp; then
  ...
  if grep -q "watchdog backend" /tmp/edge2.log /tmp/edge2.err 2>/dev/null; then ok "tty myapp → watchdog"; else bad "tty myapp fail"; cat /tmp/edge2.err | head -n 20; cat /tmp/edge2.log | head -n 10; fi
fi
...
# 4. --pty flag
echo "--- 4. --pty flag ---"
if run_timeout 5 bin/caproom --pty --limit 512 -- echo hello > /tmp/edge6.log 2> /tmp/edge6.err; then
...
if run_timeout 5 bin/caproom --pty --limit 512 -- bash -c 'tty' > /tmp/edge7.log 2> /tmp/edge7.err; then
  if grep -Eq "/dev/(ttys|pts)" /tmp/edge7.log; then ok "--pty piped still allocates pty"; else bad "--pty piped not pty"; cat /tmp/edge7.log | head -n 10; cat /tmp/edge7.err | head -n 10; fi
else
  if grep -Eq "/dev/(ttys|pts)" /tmp/edge7.log; then ok "--pty piped pty"; else bad "fail"; cat /tmp/edge7.log | head -n 10; fi
fi

# 5. CAPROOM_PTY=1
echo "--- 5. CAPROOM_PTY=1 ---"
if run_timeout 5 bash -c 'CAPROOM_PTY=1 bin/caproom --limit 512 -- echo hello' > /tmp/edge8.log 2> /tmp/edge8.err; then
...
run_timeout 5 bash -c 'python3 scripts/pty_wrapper.py bash -c "exit 42"; echo code=$?' > /tmp/edge10.log 2>&1
...
if run_timeout 5 bin/caproom --pty --limit 512 -- bash -c 'exit 42' > /tmp/edge11.log 2> /tmp/edge11.err; then
  bad "pty exit 42 should be non-zero"
else
  # bin/caproom exit code is from wait, should be 42
  run_timeout 5 bash -c 'bin/caproom --pty --limit 512 -- bash -c "exit 42"; echo ec=$?' > /tmp/edge11b.log 2>&1
...
echo "--- 8. pty tree hog ---"
if run_timeout 15 bin/caproom --pty --limit 64 --grace 2 -- node test/tree-hog.js > /tmp/edge12.log 2>&1; then
...
# 9. pty stubborn hog
echo "--- 9. pty stubborn ---"
if run_timeout 15 bin/caproom --pty --limit 64 --grace 2 -- node test/stubborn-hog.js > /tmp/edge13.log 2>&1; then
  bad "pty stubborn should be killed"
else
  if grep -q "SIGKILLed" /tmp/edge13.log || grep -q "sweep" /tmp/edge13.log || grep -q "exited cleanly" /tmp/edge13.log; then ok "pty stubborn killed (SIGKILL or clean 143)"; else bad "pty stubborn no SIGKILL"; cat /tmp/edge13.log | tail -n 20; fi
  sleep 1
  surv=$( (pgrep -f 'Buffer.alloc' 2>/dev/null || true) | wc -l | tr -d ' '); surv=${surv:-0}
...
echo "--- 10. normal hog ---"
if run_timeout 15 bin/caproom --limit 64 --grace 2 -- node test/hog.js > /tmp/edge14.log 2>&1; then
...
if run_timeout 5 bin/caproom --pty --no-intercept-tty --limit 512 -- echo hello > /tmp/edge17.log 2> /tmp/edge17.err; then
...
if run_timeout 5 bash -c 'PATH=/tmp/empty-path:/usr/bin:/bin bin/caproom --pty --limit 512 -- echo hello' > /tmp/edge18.log 2> /tmp/edge18.err; then
